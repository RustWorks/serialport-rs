...
pub mod prelude {
    ...
    pub use crate::{SerialPort, SerialPortInfo};
}
...
impl SerialPortBuilder {
    ...
    pub fn open(self) -> Result<Box<dyn SerialPort>> {
        #[cfg(unix)]
        return match posix::TTYPort::open(&self) {
            Ok(p) => Ok(Box::new(p)),
            Err(e) => Err(e),
        };

        #[cfg(windows)]
        return match windows::COMPort::open(&self) {
            Ok(p) => Ok(Box::new(p)),
            Err(e) => Err(e),
        };
...
pub fn new(port: &String) -> SerialPortBuilder {
    SerialPortBuilder {
        path: port.clone(),
        baud_rate: 9600u32,
